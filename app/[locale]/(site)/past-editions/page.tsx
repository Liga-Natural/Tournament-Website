...
import { MomentRail } from "@/components/moment-rail";
import { MomentCard } from "@/components/moment-card";
import { GalleryGrid } from "@/components/gallery-grid";
import { ArchiveTabs } from "@/components/archive-tabs";
import type { EditionAward, EventRecord } from "@/lib/types";
...
export default async function PastEditionsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [awardsBySeason, images] = await Promise.all([
    Promise.all(events.map((event) => getEditionAwards(event.id))),
    getGalleryImages(),
  ]);

  const moments: { award: EditionAward; event: EventRecord }[] = events
    .flatMap((event, i) => awardsBySeason[i].map((award) => ({ award, event })))
    .filter((m) => m.award.mvpActionPhotoUrl || m.award.mvpPhotoUrl);
  ...
  const recordBook = (
    <>
      ...
      <div className="mt-10 rounded-xl border border-gold/30 bg-gradient-to-br from-navy-raised to-navy-deep p-8">
        <div className="flex flex-col gap-6 sm:flex-row sm:items-center">
          <div className="relative aspect-[4/5] w-40 shrink-0 overflow-hidden rounded-lg border border-gold/25">
            <Image
              src="/gallery/negronis-daniele-strike.jpg"
              alt="Daniele Contarino"
              fill
              sizes="160px"
      ...
      {events.length === 0 ? (
        ...
        <div className="mx-auto max-w-4xl">
          {events.map((event, i) => {
            const awards = awardsBySeason[i];
            const isLast = i === events.length - 1;
            return (
              <div key={event.id} id={`season-${event.year}`} className="relative flex scroll-mt-24 gap-6 sm:gap-10">
                {/* Timeline rail */}
                      ...
                      <div key={award.id} className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center">
                        <div className="min-w-0 flex-1">
                          <div className="flex flex-wrap items-baseline justify-between gap-2">
                            ...
                            <span className="font-display tabular text-xl font-bold text-cream">{award.finalScoreLine}</span>
                          </div>
                          ...
                          <div className="mt-3 flex items-center gap-3 border-t border-gold/10 pt-3">
                            {award.mvpPhotoUrl && (
                              <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-full border border-gold/40">
                                <Image src={award.mvpPhotoUrl} alt={award.mvpName} fill sizes="48px" className="object-cover" />
                              </div>
                            )}
                            <div>
  ...
  const gallery = images.length === 0 ? <EmptyState message={dict.pastEditions.galleryEmpty} /> : <GalleryGrid images={images} locale={locale} />;
    ...
    <>
      ...
      {moments.length > 0 && (
        <Section className="pt-0">
          <p className="text-center text-xs font-semibold uppercase tracking-[0.3em] text-gold">{dict.pastEditions.momentKicker}</p>
          <h2 className="mt-2 text-center font-display text-2xl font-extrabold uppercase tracking-wide text-cream sm:text-3xl">
            {dict.pastEditions.momentHeading}
          </h2>
          <p className="mx-auto mt-2 max-w-lg text-center text-sm text-muted">{dict.pastEditions.momentBody}</p>
          <div className="mt-8">
            <MomentRail>
              {moments.map(({ award, event }) => (
                <MomentCard
                  key={award.id}
                  href={`#season-${event.year}`}
                  photoUrl={award.mvpActionPhotoUrl ?? award.mvpPhotoUrl}
                  eyebrow={`${event.seasonLabel} · ${award.divisionName}`}
                  name={award.mvpName}
                  meta={award.mvpTeam}
                  objectPosition={award.mvpActionPhotoUrl ? "center 18%" : "center 25%"}
                />
              ))}
            </MomentRail>
          </div>
      ...
      <Section className="pt-0">
        <ArchiveTabs
          recordBookLabel={dict.pastEditions.tabRecordBook}
          galleryLabel={dict.pastEditions.tabGallery}
